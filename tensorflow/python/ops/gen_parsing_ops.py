...
def _parse_single_sequence_example(serialized,
  ...
  Args:
    ...
    context_sparse_types: A list of `tf.DTypes` from: `tf.float32, tf.int64, tf.string`.
      A list of Ncontext_sparse types; the data types of data in
      each context Feature given in context_sparse_keys.
      Currently the ParseSingleSequenceExample supports DT_FLOAT (FloatList),
      DT_INT64 (Int64List), and DT_STRING (BytesList).
    feature_list_dense_types: A list of `tf.DTypes` from: `tf.float32, tf.int64, tf.string`.
    context_dense_shapes: A list of shapes (each a `tf.TensorShape` or list of `ints`).
      A list of Ncontext_dense shapes; the shapes of data in
      each context Feature given in context_dense_keys.
      The number of elements in the Feature corresponding to context_dense_key[j]
      must always equal context_dense_shapes[j].NumEntries().
      The shape of context_dense_values[j] will match context_dense_shapes[j].
    feature_list_dense_shapes: A list of shapes (each a `tf.TensorShape` or list of `ints`).
  ...
  Returns:
    A tuple of `Tensor` objects (context_sparse_indices, context_sparse_values, context_sparse_shapes, context_dense_values, feature_list_dense_values).
  ...
  """
  return _op_def_lib.apply_op("ParseSingleSequenceExample",
